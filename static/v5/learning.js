...
function update_url(){
    ...
    var rcid=localStorage.getItem("rcid");

    if (rcid ==undefined || rcid == "") {
        rcid           = '';
        var characters       = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789';
        var charactersLength = characters.length;
        for ( var i = 0; i < 25; i++ ) {
            rcid += characters.charAt(Math.floor(Math.random() * charactersLength));
        }
        localStorage.setItem("rcid",rcid);
    }
    //add rcid only if it is not there
    if (current_url.indexOf(rcid)==-1) {
        if (queryString.length>0)
            window.history.pushState("", "Title", current_url+"&rcid="+rcid);
        else
            window.history.pushState("", "Title", current_url+"?rcid="+rcid);
    }
    //received from someone else
    else {
        //remove current nricd
        if (queryString.length>0)
            window.history.pushState("", "Title", current_url.replace('&nrcid='+rcid,"")+"&nrcid="+rcid);
        else
            window.history.pushState("", "Title", current_url.replace('&nrcid='+rcid,"")+"?nrcid="+rcid);
    }
}
...
//===================================================================
function url_copy(input_url){
    // Create a "hidden" input
    var aux = document.createElement("input");

    // Assign it the value of the specified element
    aux.setAttribute("value", input_url);
        ...
        if (action == "enter") {
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    //check if the sc is valid
                    if (response["input_text"]!=undefined){
                        full_data = undefined;
                        ...
                        //get timing data
                        process_timing("sim", in_code_entered, "zhmp3_timing");
                        process_timing("trad", in_code_entered, "hkmp3_timing");
                        ...
                        if (response["full_data"] != undefined){
                            full_data = response["full_data"];
                        ...
                        try {
                            fetch(image_link)
                                ...
                                .then(function(blob) {
                                    ...
                                    var filesArray = [file];
                                    shareData.file = filesArray;
                                });
                        }
                        catch{
                            shareData.file = [];
                        }
                        shareData.title = story_title;
                        console.log(shareData);
                        shareData.text = trad_spaced_sentence.substring(0,40).replace(/\n/g,"")+"..."+english_meaning_article.substring(0,100)
                        // end of for sharebutton
                        ...
                        if (story_category=="寓言故事") {
                            story_category_sim = "寓言故事";
                            story_category_eng = "Fable Stories";
                        }
                        if (story_category=="安徒生童話") {
                            story_category_sim = "安徒生童话";
                            story_category_eng = "Andersen's Fairytales";
                        }
                        if (story_category=="經典名曲") {
                            story_category_sim = "经典名曲";
                            story_category_eng = "Classic Songs";
                        }
                        if (story_category=="世紀大案") {
                            story_category_sim = "世纪大案";
                            story_category_eng = "Criminal Case of the Century";
                        }
                        if (story_category=="心靈雞湯") {
                            story_category_sim = "心灵鸡汤";
                            story_category_eng = "Words for the Soul";
                        }
                        if (story_category=="星座生肖") {
                            story_category_sim = "星座生肖";
                            story_category_eng = "Chinese Zodiac";
                        }
                        if (story_category=="環遊世界") {
                            story_category_sim = "环游世界";
                            story_category_eng = "ravel around the Globe";
                        }
                        if (story_category=="當年今天") {
                            story_category_sim = "当年今天";
                            story_category_eng = "Today in History";
                        }
                        if (story_category=="兒童文學雜錦") {
                            story_category_sim = "儿童文学杂锦";
                        ...
                        {
                            story_difficulty = '<span class="txt_trad">初學</span>'
                            story_difficulty += '<span class="txt_sim">初学</span>'
                            story_difficulty += '<span class="txt_eng">Beginner</span>'
                        }
                        else if (story_difficulty=="Intermediate")
                        {
                            story_difficulty = '<span class="txt_trad">中級</span>'
                            story_difficulty += '<span class="txt_sim">中级</span>'
                            story_difficulty += '<span class="txt_eng">Intermediate</span>'
                        }
                        else if (story_difficulty=="Advanced")
                        {
                            story_difficulty = '<span class="txt_trad">高級</span>'
                            story_difficulty += '<span class="txt_sim">高级</span>'
                        ...
                        //delay the loading for 3 seconds
                        ...
                        if (mp3_request == "No") {
                            ...
                            if (full_data != undefined) {
                                display_text(story_title, story_source, story_difficulty, full_data);
                            }
                            else {
                            ...
                            if (image_link != "") {
                                ...
                                //image
                                    ...
                                    if (image_link.indexOf("mp4")>1){
                                        var video = document.createElement('video');
                                        video.src = image_link;
                                        $("#div_story_video_youtube").append(video);
                                        $("#div_story_video_youtube").show();
                                        video.controls = true;
                    ...
                    //if the sc is not valid, go to resources page
                    else {
                        window.location.href = "https://www.ricciwawa.com/resources";
                    }
                }
            })
        }
        ...
        if (!each_word_trad.includes("BBHHHAAAA70707") && !each_word_trad.includes("<BR>")) {
            ...
            for (var png_counter=0; png_counter<each_word_trad.length; png_counter++){
                ...
                if (each_word_trad[png_counter] != "")
                {
                    ...
                    if (each_word_sim[png_counter] != undefined)
...
function display_text(story_title, story_source, story_difficulty, full_data){
    span_text =""
    $("#processed_text_analysis_text").html("")
    each_word_array = []
    pevious_word = ""
...
//english_meaning_word = in_english_meaning_word.split("\n")
    ...
    //if (trad_spaced_sentence[1]=="<BR>")
    //    j=-4
    for (var counter=0; counter< full_data.length; counter++){

        each_word_sim=full_data[counter]["sim"]
        each_word_trad=full_data[counter]["trad"]
        each_word_pin_yin=full_data[counter]["pinyin"]
        each_word_eng=full_data[counter]["eng"]
        //each_english_meaning_word=english_meaning_word[counter]
        //loop through each word
        a_png_sim_url = []
        a_png_trad_url = []
        ...
        // use for adjusting spacing
        ascii_indicator = true
        //if not line break or a symbol
        //contains a Chinese character or word
        if (full_data[counter]["word"] != "<BR>" && each_word_trad.length>0) {
            //loop through the word to find the URL of each character in the word
            //a character or word may have muliple trad/sim, pinyin and meaning
            for (var png_counter=0; png_counter<each_word_trad[0].length; png_counter++){
                //puncutation

                if (each_word_trad[0][png_counter] == "，")
                {
                    //png_sim_url=png_sim_url+"<p class='class_sim_png'>，</p>";
                    //png_trad_url=png_trad_url+"<p class='class_trad_png'>，</p>";
                    each_word_pin_yin[0][png_counter]=""
                }
                else if (each_word_trad[png_counter] == "。")
                {
                    //png_sim_url=png_sim_url+"<p class='class_sim_png'>。</p>";
                ...
                if (each_word_trad[0][png_counter] != "")
                {
                    ...
                    try {
                        sim_unicode = encodeURIComponent(each_word_sim[0][png_counter]).replace(/\%/g,"x").toLowerCase()
                    }
                    catch {
                        //check if the character also has problen
                        if (png_counter<each_word_sim.length) {
                            try {
                                test=encodeURIComponent(each_word_sim[0][png_counter]).replace(/\%/g,"x").toLowerCase()
                            }
                            //error or emoji
                            catch {
                                sim_word = each_word_sim[0][png_counter] + each_word_sim[0][png_counter+1];
                                emoji_flag = true
                                //emoji is double chart, so do not loop
                            }
                        }
                    ...
                    try {
                        trad_unicode = encodeURIComponent(each_word_trad[0][png_counter]).replace(/\%/g,"x").toLowerCase()
                    }
                    catch {
                        //check if the character also has problen
                        if (png_counter<each_word_trad.length) {
                            try {
                                test=encodeURIComponent(each_word_trad[0][png_counter]).replace(/\%/g,"x").toLowerCase()
                            }
                            //error or emoji
                            catch {
                                trad_word = each_word_trad[0][png_counter] + each_word_trad[0][png_counter+1];
                                emoji_flag = true
                                //emoji is double chart, so do not loop
                            }
                        }
                    ...
                    //don't get the image for English and puncutation
                    sim_unicode = sim_unicode.trim()
                    if (each_word_sim[0][png_counter] != undefined){
                        if (each_word_sim[0][png_counter] != undefined)
                            if (a_chinese_punctuation.includes(each_word_sim[0][png_counter].trim())==true)
                                flag_chinese_punctuation = true;
                    }
                    s_pin_yin = (each_word_pin_yin[0])[png_counter];
                    if (s_pin_yin == undefined)
                        s_pin_yin=""
                    else {
                        s_pin_yin_length = s_pin_yin.length;
                        s_pin_yin_char = "";
                        s_pin_yin_tone = s_pin_yin.substring(s_pin_yin_length-1);
                        //to ensure the length is more than 1
                        if (s_pin_yin_length>1) {
                            if (s_pin_yin.indexOf("iu")!=-1){
                                if (s_pin_yin_tone == "1") s_pin_yin = s_pin_yin.replace("iu","iū")
                                if (s_pin_yin_tone == "2") s_pin_yin = s_pin_yin.replace("iu","iú")
                                if (s_pin_yin_tone == "3") s_pin_yin = s_pin_yin.replace("iu","iǔ")
                                if (s_pin_yin_tone == "4") s_pin_yin = s_pin_yin.replace("iu","iù")
                                if (s_pin_yin_tone == "5") s_pin_yin = s_pin_yin.replace("iu","iu")
                            }
                            else if (s_pin_yin.indexOf("a")!=-1){
                                if (s_pin_yin_tone == "1") s_pin_yin = s_pin_yin.replace("a","ā")
                                if (s_pin_yin_tone == "2") s_pin_yin = s_pin_yin.replace("a","á")
                                if (s_pin_yin_tone == "3") s_pin_yin = s_pin_yin.replace("a","ǎ")
                                if (s_pin_yin_tone == "4") s_pin_yin = s_pin_yin.replace("a","à")
                                if (s_pin_yin_tone == "5") s_pin_yin = s_pin_yin.replace("a","a")
                            }
                            else if (s_pin_yin.indexOf("o")!=-1){
                                if (s_pin_yin_tone == "1") s_pin_yin = s_pin_yin.replace("o","ō")
                                if (s_pin_yin_tone == "2") s_pin_yin = s_pin_yin.replace("o","ó")
                                if (s_pin_yin_tone == "3") s_pin_yin = s_pin_yin.replace("o","ǒ")
                                if (s_pin_yin_tone == "4") s_pin_yin = s_pin_yin.replace("o","ò")
                                if (s_pin_yin_tone == "5") s_pin_yin = s_pin_yin.replace("o","o")
                            }
                            else if (s_pin_yin.indexOf("e")!=-1){
                                if (s_pin_yin_tone == "1") s_pin_yin = s_pin_yin.replace("e","ē")
                                if (s_pin_yin_tone == "2") s_pin_yin = s_pin_yin.replace("e","é")
                                if (s_pin_yin_tone == "3") s_pin_yin = s_pin_yin.replace("e","ě")
                                if (s_pin_yin_tone == "4") s_pin_yin = s_pin_yin.replace("e","è")
                                if (s_pin_yin_tone == "5") s_pin_yin = s_pin_yin.replace("e","e")
                            }
                            else if (s_pin_yin.indexOf("i")!=-1){
                                if (s_pin_yin_tone == "1") s_pin_yin = s_pin_yin.replace("i","ī")
                                if (s_pin_yin_tone == "2") s_pin_yin = s_pin_yin.replace("i","í")
                                if (s_pin_yin_tone == "3") s_pin_yin = s_pin_yin.replace("i","ǐ")
                                if (s_pin_yin_tone == "4") s_pin_yin = s_pin_yin.replace("i","ì")
                                if (s_pin_yin_tone == "5") s_pin_yin = s_pin_yin.replace("i","i")
                            }
                            else if (s_pin_yin.indexOf("u")!=-1){
                                if (s_pin_yin_tone == "1") s_pin_yin = s_pin_yin.replace("u","ū")
                                if (s_pin_yin_tone == "2") s_pin_yin = s_pin_yin.replace("u","ú")
                                if (s_pin_yin_tone == "3") s_pin_yin = s_pin_yin.replace("u","ǔ")
                                if (s_pin_yin_tone == "4") s_pin_yin = s_pin_yin.replace("u","ù")
                                if (s_pin_yin_tone == "5") s_pin_yin = s_pin_yin.replace("u","u")
                            }
                            //replace the tone indicator
                            s_pin_yin=s_pin_yin.substring(0,s_pin_yin_length-1);
                        }
                    }
                    //convert s_pinyin tone indicator
                    if (sim_unicode.length == 9 && !emoji_flag && flag_chinese_punctuation==false){ //print non-English and non-number
                        //png_sim_url=png_sim_url+"<img class='class_sim_png' src='/get_char?char_id="+sim_unicode.toLowerCase()+"_radical_50.png' alt='" +  each_word_sim[png_counter] + "'>"
                        //png_trad_url=png_trad_url+"<img class='class_trad_png' src='/get_char?char_id="+trad_unicode+"_radical_50.png' alt='" +  each_word_trad[png_counter] + "'>"
                        if (each_word_pin_yin[0][png_counter]==undefined)
                            each_word_pin_yin[png_counter]=""
                        ...
                        png_sim_url=png_sim_url+"<div class='class_div_sim' id='char_id_sim_" + (j+png_counter) + "' style='display:inline-block;'><span><span class='class_pin_yin'>" + s_pin_yin + "</span><span class='class_normal_text class_sim_text'>" + each_word_sim[0][png_counter] + "</span></span><img class='class_sim_png' src='https://f001.backblazeb2.com/file/RicciwawaTempPublic/char/"+sim_unicode.toLowerCase()+".svg' alt='" +  each_word_sim[png_counter] + "' onerror='tryAgain_image(this)' /></div>"
                        png_trad_url=png_trad_url+"<div class='class_div_trad' id='char_id_trad_" + (j+png_counter) + "' style='display:inline-block;'><span><span class='class_pin_yin'>" + s_pin_yin + "</span><span class='class_normal_text class_trad_text'>" + each_word_trad[0][png_counter] + "</span></span><img class='class_trad_png' src='https://f001.backblazeb2.com/file/RicciwawaTempPublic/char/"+trad_unicode.toLowerCase()+".svg' alt='" +  each_word_trad[0][png_counter] + "' onerror='tryAgain_image(this)'/></div>";
                        ascii_indicator = false
                    }
                    else {
                    // if (emoji_flag) {
                        png_sim_url=png_sim_url+"<div class='class_div_sim' id='char_id_sim_" + (j+png_counter) + "' style='display:inline-block;'><span><span class='class_pin_yin'>&nbsp;</span><span class='class_normal_text class_sim_text class_punctuation'>" + each_word_sim[0][png_counter] + "</span></span><span class='class_sim_png class_punctuation'>" + each_word_sim[0][png_counter]  + "</span></div>"
                        png_trad_url=png_trad_url+"<div class='class_div_trad' id='char_id_trad_" + (j+png_counter) + "' style='display:inline-block;'><span><span class='class_pin_yin'>&nbsp;</span><span class='class_normal_text class_trad_text class_punctuation'>" + each_word_trad[0][png_counter] + "</span></span><span class='class_trad_png class_punctuation'>" + each_word_trad[0][png_counter]  + "</span></div>";

                    }
                    if (emoji_flag == true){
                        png_sim_url=png_sim_url+"<div class='class_div_sim' id='char_id_sim_" + (j+png_counter) + "' style='display:inline-block;'><span><span class='class_pin_yin'>&nbsp;</span><span class='class_normal_text class_sim_text class_punctuation'>" + each_word_sim[0] + "</span></span><span class='class_sim_png class_punctuation'>" + each_word_sim[0]  + "</span></div>"
                        png_trad_url=png_trad_url+"<div class='class_div_trad' id='char_id_trad_" + (j+png_counter) + "' style='display:inline-block;'><span><span class='class_pin_yin'>&nbsp;</span><span class='class_normal_text class_trad_text class_punctuation'>" + each_word_trad[0] + "</span></span><span class='class_trad_png class_punctuation'>" + each_word_trad[0]  + "</span></div>";
            ...
            if (!ascii_indicator) {
                ...
                if (flag_chinese_punctuation == true)
                    ...
                    padding_left_value = "0.8em"
                span_text = span_text + '<a href="#/" class="link_popover" data-html="true" data-toggle="popover" data-placement="bottom" data-content="' + each_word_trad[0] + "/" + each_word_sim[0] + "<BR>" + each_word_eng + '">' +
                "<div class='span_word' style='white-space: nowrap;width:auto;padding-left:" + padding_left_value + ";display:inline-block; text-align:center;' id='word_" + counter + "'>" + png_sim_url + png_trad_url + "</div></a>"
            }
            else {
                if (flag_chinese_punctuation == true)
                    span_text = span_text + "<div class='span_word' style='white-space: nowrap;width:auto;padding-left:0.1em;display:inline-block; text-align:center;padding-top:1em;' id='word_" + counter + "'>" + png_sim_url + png_trad_url + "</div>"

                else
                    span_text = span_text + "<div class='span_word' style='white-space: nowrap;width:auto;padding-left:0.8em;display:inline-block; text-align:center;padding-top:1em;' id='word_" + counter + "'>" + png_sim_url + png_trad_url + "</div>"
            }
        }
        else {
            if  (full_data[counter]["word"] == "<BR>")
            {
                span_text = span_text + "<BR><div style='display:none' id='word_" + counter + "'><span id='char_id_trad_"+j+"'><span id='char_id_sim_"+j+"'></div>"
            }
            else
                //span_text = span_text + full_data[counter]["word"]
                span_text = span_text + "<div class='span_word' style='white-space: nowrap;width:auto;padding-left:0.1em;display:inline-block; text-align:center;' id='word_" + counter + "'><span class='class_normal_text'>" + full_data[counter]["word"]  + "</span><span id='char_id_trad_"+j+"'></span><span id='char_id_sim_"+j+"'></span></div>"

        }
        //span_text = span_text + "<div class='dis_eng'>" + each_word_eng +"</div>"
        if (full_data[counter]["word"]!= "<BR>")
            j = j + full_data[counter]["word"].length;
        else
            j= j + 1; //match Azure TTS counter
     }
     //==============================
     // temporary
     $('.class_meaning').hide();
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            if (window.location.href.indexOf("riccinalysis")>0)
                ...
                $("#span_public_article_code").html('https://www.ricciwawa.com/' + response["target_lang"] + '/?sc='+response["public_code"]);
            }
            else
...
timing_data_trad=[];
timing_data_sim=[];
function process_timing(trad_or_sim, sc, file_type){
    if (sc!=undefined)
    {
        $.ajax({
            url: "/get_audio64?sc="+sc+"&f="+file_type,
            type: 'GET',
            success: function (response) {
                array_timing = response.split("\\n");
                console.log(array_timing);
                array_timing_word = [];
                char_offset = 0;
                cumumlative_substract = 0;
                $("#div_debug2").text(response.substring(0,30));
                for (var counter=0; counter< array_timing.length; counter++){
                    try{
                        json_data = JSON.parse(array_timing[counter]);
                        ...
                        if (counter>100 && counter<105)
                            console.log(counter, json_data, temp_word, json_data["char_start"], cumumlative_substract,  char_offset)
/*
...
//                            cumumlative_substract = cumumlative_substract + 1;
                        if (counter>0) {
                            ...
                            console.log(counter, char_offset, json_data["char_start"] - char_offset, json_data["char_end"], json_data["audio_start"], json_data["audio_end"], temp_word);
                        }
...
function show_audio_time(trad_or_sim){
    if (trad_or_sim == "trad"){
        ...
        $("#div_debug").text(audio_currenttime);
    }
    else {
...
//only works for https
    $("#share_button_url").click(function(e){
        if (navigator.share) {
            ...
            .catch((e) => {
                /*
                    url_copy(window.location.href);
                    $("#span_link_copied").show();
                    setTimeout(() => {
                        $("#span_link_copied").hide();
                    }, 3000);
                */
                }
            )
...
function scrollFunction() {
    if (document.body.scrollTop > 300 || document.documentElement.scrollTop > 300) {
        ...
        $("#nav_setting_bar").css("margin-top","0px");
    }
}
